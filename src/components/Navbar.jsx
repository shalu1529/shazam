...
function Navbar() {
  ...
  // Check if the user is authenticated by checking localStorage for the auth token
  useEffect(() => {
  ...
  // Handle logout - remove token from localStorage and redirect
  const handleLogout = () => {
  ...
  return (
    <nav className={`${bgColor} ${textColor} flex justify-between items-center px-8 py-3 fixed top-0 w-full z-50 transition-all duration-300 mb-6`}>
      ...
      <div className="flex items-center space-x-4">
        ...
        {/* Hamburger Menu Icon or Close Icon - Mobile view */}
        <div className="md:hidden">
          {isMobileMenuOpen ? (
            <FaTimes
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-0 left-0 w-full h-screen bg-black text-white flex flex-col items-center justify-center space-y-8 md:hidden">
          <Link to="/get-app" className="text-xl font-bold hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
            GET THE APP
          </Link>
          <Link to="/charts" className="text-xl font-bold hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
            CHARTS
          </Link>
          <Link to="/radio" className="text-xl font-bold hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
            RADIO SPINS
          </Link>
          <Link to="/wishlist" className="text-xl font-bold hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
            PLAYLIST
          </Link>
        </div>
      )}
